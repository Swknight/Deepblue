...
class TLSTM(object):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('fcl'):
            def get_weights(shape,name='',lambd=0.01):
                var = tf.Variable(tf.random_normal(shape),name=name)
                tf.add_to_collection('losses', tf.contrib.layers.l2_regularizer(lambd)(var))
                return var

            weights = {
                # 'mat1': tf.Variable(tf.random_normal([timesteps*num_hidden, 200])),
                'fc1': get_weights([hidden_shape, 100], name='w1'),
                # tf.Variable(tf.random_normal([hidden_shape, 100]), name='w1'),
                'fc2': get_weights([100, 1], name='w2'),
            }
            biases = {
    ...
    def train(self, epoch=10):
        ...
        in_inputs_sequence= np.load('../data/lstm_train_inputs_sequence_nrl_except_outlier_except0.npy')
        in_targets= np.load('../data/lstm_train_targets_nrl_except_outlier_except0.npy')
        in_delt_time = np.load('../data/lstm_train_delt_time_nrl_except_outlier_except0.npy')
        in_y = np.load('../data/lstm_train_y_nrl_except_outlier_except0.npy')
        in_time_sequence = np.load('../data/lstm_train_time_sequence_nrl_except_outlier_except0.npy').astype(np.float32)
        in_inputs = np.load('../data/lstm_train_inputs_nrl_except_outlier_except0.npy')
