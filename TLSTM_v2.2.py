...
class TLSTM(object):
    ...
    def train(self, epoch=10):
        ...
        # in_inputs_sequence= np.load('../data/lstm_train_inputs_sequence.npy')
        # in_targets= np.load('../data/lstm_train_targets.npy')
        # in_delt_time = np.load('../data/lstm_train_delt_time.npy')
        # in_y = np.load('../data/lstm_train_y.npy')
        # in_time_sequence = np.load('../data/lstm_train_time_sequence.npy').astype(np.float32)
        # in_inputs = np.load('../data/lstm_train_inputs.npy')
        # num_sample = int(229617)
        ...
        with tf.Session() as sess:
            ...
            for one_epoch in range(epoch):
                ...
                for oneepoch_data in data_loader(32):
                    ...
                    adamTrain, lossValue,mse = sess.run([self.adam_train_op, self.loss,self.mse], feed_dict={self.inputs: __inputs,
                    ...
                    mean_loss += int(mse)
                    print("------------------------step" + str(step) + "-------loss" + str(mse)+ "-------loss" + str(__y))
                    step += 1
                    pass
                print("---------------------------Loss" + str(int(mean_loss/1435)))
                # print(step,"")
            merged = tf.summary.merge_all()
            # tf.summary.FileWriter()将所有信息写入文件
            writer = tf.summary.FileWriter('./tensorflow1.1_logs', sess.graph)
