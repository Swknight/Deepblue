...
class TLSTMCell(RNNCell):
    ...
    def call(self, inputs, state):
        ...
        new_c0 = c0 * math_ops.exp(-1 * delt_t) * sigmoid(f00 + self._forget_bias) + (1 - math_ops.exp(
            -1 * delt_t)) * sigmoid(i00) * leakyRelu(j00)
        # new_c0 = c0 * sigmoid(f00 + self._forget_bias)

        # print('state 2.2')
        new_c0 = new_c0 * sigmoid(f10 + self._forget_bias) + sigmoid(i10) * leakyRelu(j10)

        # print('state 2.3')
        new_h0 = leakyRelu(new_c0) * sigmoid(output_x)
