...
class TLSTM(object):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('TLSTM_X'):
            ...
            #Y_lstm h 经过一个fc之后再加入 T_lstm 32x6x128 = 24576
            with tf.variable_scope("joint_fcl"):
                w_joint = tf.Variable(tf.random_normal([128,128],name = "w_joint"))
                b_joint = tf.Variable(tf.random_normal([128]),name = "b_joint")
                outputs0 = tf.reshape(outputs0,[1,24576])
                part = tf.split(outputs0,192,1)
                newoutputs = tf.matmul(part[0],w_joint)+b_joint
                i = 1
                while i < 192:
                    newpart = tf.matmul(part[i],w_joint)+b_joint
                    newoutputs = tf.concat([newoutputs,newpart],1)
                    i+=1
            newoutputs = tf.reshape(newoutputs,[32,6,128])
            y_state = tf.concat(
                [tf.convert_to_tensor([[begin_state]] * self._batch_size), tf.convert_to_tensor(newoutputs)], 1)
        ...
        self.loss = tf.nn.log_poisson_loss(log_input=output_logit, targets=self.y) + regularization_cost



        tf.add_to_collection('losses', self.loss)
        self.mse = tf.nn.l2_loss(math_ops.exp(output_logit)-self.y)
        # self.mae =tf.losses.absolute_difference(self.y,math_ops.exp(output_logit))
        self.mae = tf.reduce_mean(tf.abs(tf.divide(tf.subtract(math_ops.exp(output_logit),self.y),self.y)))
    ...
    def train(self, epoch=10):
        ...
        in_inputs_sequence= np.load('/media/dell_1/wenjian/lstm_train_inputs_sequence_nrl_except_outlier_except0.npy')
        in_targets= np.load('/media/dell_1/wenjian/lstm_train_targets_nrl_except_outlier_except0.npy')
        in_delt_time = np.load('/media/dell_1/wenjian/lstm_train_delt_time_nrl_except_outlier_except0.npy')
        in_y = np.load('/media/dell_1/wenjian/lstm_train_y_nrl_except_outlier_except0.npy')
        in_time_sequence = np.load('/media/dell_1/wenjian/lstm_train_time_sequence_nrl_except_outlier_except0.npy').astype(np.float32)
        in_inputs = np.load('/media/dell_1/wenjian/lstm_train_inputs_nrl_except_outlier_except0.npy')
        num_sample = in_y.shape[0] # total samples
        ...
        def data_loader_test(batch_size):
            idx = list(range(int(num_sample * (1 - split) )))
            random.shuffle(idx)
            for i in range(0, int(num_sample * split), batch_size):
                j = np.array(idx[i:min(i + batch_size, int(num_sample * split))])
                if len(j) != batch_size:
                    break
                yield  test_inputs_sequence[j],  test_targets[j],  test_delt_time[j],  test_y[j],  test_time_sequence[
                    j],  test_inputs[j]
        ...
        with tf.Session() as sess:
            ...
            for one_epoch in range(epoch):
                ...
                for oneepoch_data in data_loader(32):
                    ...
                    adamTrain, lossValue,mae,prediction,y = sess.run([self.adam_train_op, self.loss,self.mae, self.action_predict_label,self.y], feed_dict={self.inputs: __inputs,
                    ...
                    mean_loss += float(mae.mean())
                    self.all_prediction.extend(prediction[:,0])
                    self.all_y.extend(y[:,0])
                    step += 1
                    # print("step" + str(step))
                    pass
                print("---------------------------Train Loss mae" + str(float(mean_loss/(num_sample*split/self._batch_size))))      
                print("---------------------------Train Loss spearman" + str(scipy.stats.spearmanr(self.all_prediction,self.all_y)[0]))
                mean_loss = 0
                self.all_prediction = []
                self.all_y = []
                for oneepoch_data in data_loader_test(32):
                    __inputs_sequence, __targets, __delt_time, __y, __time_sequence, __inputs = oneepoch_data
                    mae,y,prediction = sess.run([self.mae,self.y,self.action_predict_label], feed_dict={self.inputs: __inputs,self.inputs_sequence: __inputs_sequence,
                                                                                                self.targets: __targets,
                                                                                                self.inputs_keep_prob: 0.8,
                                                                                                self.tlstm_keep_prob: 0.8,
                                                                                                self.delt_time: __delt_time,
                    ...
                    step += 1
                    pass
                print("---------------------------Test Loss mae" + str(float(mean_loss/(num_sample*split/self._batch_size))))      
                print("---------------------------Test Loss spearman" + str(scipy.stats.spearmanr(self.all_prediction,self.all_y)[0]))
                # print(step,"")
            merged = tf.summary.merge_all()
            # tf.summary.FileWriter()将所有信息写入文件
            writer = tf.summary.FileWriter('./tensorflow1.1_logs', sess.graph)
